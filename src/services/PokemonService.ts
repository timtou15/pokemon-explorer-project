...
export class PokemonService implements IPokemonService {
  ...
  async getPokemonSpecies(id: number) {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(`Espécie não encontrada para Pokémon ID: ${id}`);
        const emptySpecies = {
          flavor_text_entries: [],
          genera: [],
          habitat: null,
      ...
      console.error(`Erro ao buscar espécie do Pokémon ID ${id}:`, error);
      const emptySpecies = {
        flavor_text_entries: [],
        genera: [],
        habitat: null,
  ...
  async getPokemonBatch(ids: number[]) {
    const promises = ids.map(async (id) => {
      try {
        ...
        console.error(`Erro ao buscar Pokémon ID ${id}:`, error);
        return null;
      }
    });
  ...
  async getAbilityDescription(abilityName: string, language: string = 'en'): Promise<string> {
    ...
    try {
      ...
      if (language === 'pt') {
        // Traduções manuais para habilidades mais comuns em português
        const manualTranslations: {[key: string]: string} = {
          ...
          'wonder-guard': 'Só recebe dano de ataques super eficazes.',
          ...
          'shield-dust': 'Protege contra efeitos adicionais de ataques.',
          'own-tempo': 'Protege contra confusão.',
          ...
          'solar-power': 'Aumenta Ataque Especial no sol, mas perde HP.',
        ...
        if (manualTranslations[abilityName]) {
          ...
          // Se não encontrou, tentar flavor text entries em português
        ...
        // Para inglês, buscar diretamente
        const enEntry = abilityData.effect_entries?.find(
          (entry: any) => entry.language.name === 'en'
        );
        if (enEntry) {
